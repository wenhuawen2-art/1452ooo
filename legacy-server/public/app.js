...
const legacyReadOnly = Boolean(window.SUIXING_READ_ONLY);
if (legacyReadOnly) document.body.classList.add("legacy-readonly");
...
let token = localStorage.getItem("suixing-token") || "",
  trip = null,
  trips = [],
  tab = "today",
  scope = "公共",
...
async function api(path, data) {
  ...
  if (!r.ok) {
    ...
    throw Object.assign(Error(v.error || "操作失败"), { status: r.status });
  }
  return v;
}
function remember(v) {
  token = v.token;
  localStorage.setItem("suixing-token", token);
  trip = v.trip;
