...
const PERIODS = [{ name: "上午", range: "08:00—13:00" }, { name: "下午", range: "13:00—18:00" }, { name: "晚上", range: "18:00—23:00" }];
...
Page({
  data: {
    ...
    weather: null, weatherLoading: false, periods: [], hotel: null, checklistGroups: [], templates: [],
  ...
  openItem(event) { const id = event.currentTarget.dataset.id; const current = this.data.trip.items.find((entry) => entry.id === id) || {}; const groups = this.data.trip.categories.filter((entry) => this.data.scope === "公共" ? !entry.owner : entry.owner === this.data.trip.me); this.setData({ modal: { type: "item", edit: !!id, title: id ? "编辑清单条目" : "新增条目", id, categories: groups, categoryIndex: Math.max(0, groups.findIndex((entry) => entry.id === (current.categoryId || event.currentTarget.dataset.category))), values: { title: current.title || "", categoryId: current.categoryId || event.currentTarget.dataset.category || groups[0]?.id || "", key: !!current.key, remind: current.remind || "", linkedDate: current.linkedDate || "" } } }); },
  openCategory() { this.setData({ modal: { type: "category", title: "新建分类", values: { name: "" } } }); },
  async openTemplates() { try { const templates = this.data.templates.length ? this.data.templates : await this.call("listChecklistTemplates"); this.setData({ templates, modal: { type: "templates", title: "导入清单模板", values: {} } }); } catch (error) { this.notify(error.message); } },
  ...
  async scanWebLogin() { try { const scan = await wx.scanCode({ scanType: ["qrCode"] }); const match = String(scan.result || "").match(/^xiangye-login:([a-f0-9]{32}):([a-f0-9]{32})$/); if (!match) throw new Error("这不是向野网页登录码"); const confirmed = await new Promise((resolve) => wx.showModal({ title: "登录网页版", content: "确认允许当前网页使用你的向野账号？", success: (result) => resolve(result.confirm) })); if (!confirmed) return; await this.call("confirmWebLogin", { sessionId: match[1], secret: match[2] }); this.notify("网页版登录成功"); } catch (error) { if (!/cancel/i.test(error.errMsg || "")) this.notify(error.message || "扫码失败"); } },
