...
function eventsCard(d) {
  ...
  return `<div class="itinerary-list">${schedulePeriods.map((period) => {
    ...
    return `<article class="card itinerary-card period-card period-${period.id}"><div class="itinerary-head period-card-head"><h3>${period.name}行程</h3><span class="period-count">${periodItems.length} 项</span></div><div class="itinerary-divider" aria-hidden="true"></div><div class="period-timeline">${periodItems.map((entry) => `<section class="period-event"><span class="journey-dot" aria-hidden="true"></span><div class="period-event-body"><div class="period-event-head"><div><div class="period-event-time">${esc(entry.segmentStart)} <span>—</span> ${esc(entry.segmentEnd)}</div><h4>${esc(entry.title)}</h4></div><div class="period-event-actions"><button class="itinerary-edit" data-action="event" data-id="${entry.id}" aria-label="编辑 ${esc(entry.title)}" data-write>${icon("edit")}</button>${mapAction(entry)}</div></div>${eventAddress(entry)}${routePlaces(entry)}${entry.note ? `<p class="itinerary-note">${esc(entry.note)}</p>` : ""}</div></section>`).join("")}</div></article>`;
  }).join("")}</div>`;
}
